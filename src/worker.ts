...
import {getPendingJobs,updateJobStatus,incrementAttempts}from "./db/queries/jobs.js"
...
switch (pipe.action) {
  ...
  try{
    ...
    await  updateJobStatus(job.id  as string,"failed" );
     ...
     await updateJobStatus(job.id  as string,"success" );   
